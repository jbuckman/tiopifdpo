...
#
#################################################################################################################
def dqn(env, eval_env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    ...
    data_return_init = []
    frame_stamp_init = []
    try: os.makedirs("logs/dqn/")
    except: pass
    try: os.makedirs("rollouts/minatar/")
    except: pass
    try: os.makedirs("policies/dqn/minatar/")
    except: pass
    logname = f"logs/dqn/{env.game}.csv"
        ...
        while(not is_terminated) and t <= NUM_FRAMES:
            ...
            if t > 0 and t % 10000 == 0:
                logging.info("Episode " + str(e) + " | Return: " + str(G) + " | Avg return: " +
                    ...
                    (time.time() - t_start) / max(1, t - REPLAY_START_SIZE)))

                G_test = 0.
                D_test = 1.
                # Initialize the environment and start state
                s_test = eval_env.reset().obs.float()
                # s = get_state(env.get_state())
                while (isinstance(D_test, float) or D_test.sum() > 0.):
                    # Generate data
                    _outcome, action = world_dynamics(t, replay_start_size, num_actions, s_test, eval_env, policy_net, test=True)
                    s_prime_test, reward, discount = _outcome.obs.float(), _outcome.reward, _outcome.discount
                    G_test += reward * D_test
                    D_test *= discount
            ...
            # Generate data
            _outcome, action = world_dynamics(t, replay_start_size, num_actions, s, env, policy_net)
            # save to rb
            dataset.add_data(obs=last_obs, action=action, reward=_outcome.reward, discount=_outcome.discount, initial=_outcome.initial)
            if t in [1e4, 2e4, 5e4, 1e5, 2e5, 5e5, 1e6, 2e6, 5e6]:
                torch.save(policy_net.state_dict(), f"policies/dqn/minatar/{env.game}_{t:07}.params")
                with open(f"rollouts/minatar/dqn_{env.game}_{int(t):07}.rb", "wb") as f:
...
def main():
    ...
    with launch_ipdb_on_exception():
        dqn(env, eval_env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
